...
class JKAIIntelligenceEngine:
    def __init__(self):
        ...
        self.executor_url = os.getenv('EXECUTOR_URL', 'http://ai-executor:8000')
        self.planner_url = os.getenv('PLANNER_URL', 'http://ai-planner:8000')
        self.current_service_url = None # 🏛️ [IDENTITY-TAG]: Sẽ được set bởi service thưa Master
        self.is_brain_service = False # Legacy flag
        
        if not self.ollama_host.startswith('http'):
            self.ollama_host = f"http://{self.ollama_host}"
        
        # 🛡️ [WINDOWS-RELIABILITY]: Chuyển hướng 0.0.0.0 về 127.0.0.1 thưa Master
        if "0.0.0.0" in self.ollama_host:
            self.ollama_host = self.ollama_host.replace("0.0.0.0", "127.0.0.1")
            
        # Đảm bảo port nếu thiếu thưa Master
        ...
        self.global_params = {}
        # 🔒 [ELITE REDIS LOCKS]: Khóa toàn cục hệ thống thưa Master
        self.lock_timeout = 180 # 3 phút tối đa chờ nơ-ron
        
        # 🚀 [QUANTUM-LEAP v31.0]: Thực thi Processor Affinity thưa Master
        self._apply_hardware_affinity()
        self.agent_profiles_cache = {}

    def _load_agent_profiles(self):
        """📂 [SINGULARITY-LOAD]: Nạp toàn bộ tinh hoa Đặc vụ vào RAM thưa Master."""
    ...
    async def _acquire_neural_lock(self, lock_name, timeout=None):
        """💎 [NEURAL-LOCK]: Giao thức Xếp hàng nơ-ron chuẩn Elite thưa Master."""
        r = self._get_redis()
        ...
        while time.time() - start_time < effective_timeout:
            # Sử dụng Redis SETNX để làm khóa thưa Master
    ...
    async def _release_neural_lock(self, lock_name):
        """🔓 Giải phóng lãnh thổ nơ-ron thưa Master."""
        r = self._get_redis()
        if r: r.delete(f"lock:{lock_name}")
        return True

    async def _enter_neural_gate(self, model_name):
        """💎 [NEURAL GATE]: Mở cổng nơ-ron cư trú thưa Master."""
        pass

    async def _exit_neural_gate(self, model_name):
        """💎 [NEURAL GATE]: Đóng cổng nơ-ron cư trú thưa Master."""
        pass

    def _get_client(self):
        """Khởi tạo hoặc trả về Client dùng chung để tối ưu TCP thưa Master."""
    ...
    def _get_redis(self):
        """Khởi tạo kết nối Redis lười biếng để phát log tư duy thưa Master."""
    ...
    def _publish_thought(self, role, thought, task_id="system", stream_id=None):
        """Phát sóng luồng tư duy lên Dashboard - Elite v31.2 thưa Master."""
        try:
            r = self._get_redis()
            if r and thought:
                msg = f"🧠 [{role} THOUGHT]: {thought.strip()}"
                display_tag = role.upper()
                payload = {
                    "tag": display_tag, "msg": msg, "ts": time.time(), "task_id": task_id or "system"
                }
                if stream_id:
                    payload["id"] = stream_id
                log_payload = json.dumps(payload, ensure_ascii=False)
                # 📡 [BROADCAST]: Phát tín hiệu nơ-ron đa điểm thưa Master.
    ...
    def publish_mission_log(self, tag, msg, task_id="system", trace_id=None, stealth=False):
        """
        🧠 [STRATEGIC-LOG]: Giao thức Nhật ký Chiến lược thưa Master.
    ...
    def publish_progress(self, pct, msg, task_id="system"):
        """Giao thức Phát sóng Tiến độ chuẩn v12.0 thưa Master."""
        r = self._get_redis()
        if r:
            try:
                payload = json.dumps({
    ...
    def _parse_rules(self):
        try:
            ...
            for line in lines:
                line = line.strip()
                # 💎 [GLOBAL PARAMS]: Trích xuất thông số tổng thể từ Section 1 thưa Master
                if line.startswith('- **') and ':' in line:
                    # Hỗ trợ cả **KEY**: và **KEY:** thưa Master
                ...
                if len(parts) >= 3 and headers:
                    ...
                    try:
                        # 💎 GIẢI MÃ THAM SỐ ELITE: Chỉ nạp những gì Master chỉ định thưa Master.
                        opts = {}
                        
                        ctx = get_safe_int('NUM_CTX', -1)
                        if ctx != -1: opts["num_ctx"] = ctx
                        
                        threads = get_safe_int('NUM_THREAD', -1)
                        if threads != -1: opts["num_thread"] = threads
                        ...
                        # 💎 [PROFILE-MERGING]: Hợp nhất Preset vào Role thưa Master
                        ...
                        if active_profile and active_profile in self.profiles:
                            ...
                            # Chỉ lấy từ Profile nếu trong bảng Mapping để N/A hoặc rỗng thưa Master
                            if ctx == -1: opts["num_ctx"] = preset["num_ctx"]
                            if threads == -1: opts["num_thread"] = preset["num_thread"]
                        ...
                        # Đọc Keep-alive nếu có, mặc định 5m thưa Master
                        keep_alive = get_safe_str('KEEP_ALIVE', '5m')
                        new_r_cache[role_key] = {'model': model_name, 'options': opts, 'keep_alive': keep_alive}
                    except Exception as e:
    ...
    async def warmup_all_models(self):
        """🚀 GIAO THỨC TRIỆU HỒI TOÀN QUÂN: Nạp sẵn các model chiến lược thưa Master."""
        ...
        logger.info(f"🛡️ [GUARDIAN] Khởi động Giao thức Triệu hồi ({len(unique_models)} model)...")
        client = self._get_client()
        
        # 🕵️ [STEP-1]: Tầm soát Kho tàng (Tags) thưa Master
        try:
            tags_resp = await client.get(f"{self.ollama_host}/api/tags")
            available_tags = [m['name'] for m in tags_resp.json().get('models', [])] if tags_resp.status_code == 200 else []
        except: available_tags = []

        # 🧠 [STEP-2]: Giám định Hiện diện (PS) thưa Master
        try:
            ps_resp = await client.get(f"{self.ollama_host}/api/ps")
            loaded_models = [m['name'] for m in ps_resp.json().get('models', [])] if ps_resp.status_code == 200 else []
        except: loaded_models = []

        for model, info in unique_models.items():
            cfg = info['cfg']
            role = info['role']
            
            # 🛡️ [ELITE-FILTER]: Chỉ triệu hồi những nơ-ron cần thiết thưa Master
            keep_alive_raw = str(cfg.get('keep_alive', '5m')).strip()
            if keep_alive_raw == '0' or model.lower() in ['faster-whisper', 'sdxl-turbo-rocm']:
                continue

            # 🛑 KIỂM TRA SỰ TỒN TẠI thưa Master
            if available_tags and not any(model in t for t in available_tags):
                logger.error(f"❌ [GUARDIAN] KHÔNG TÌM THẤY ĐẶC VỤ `{model}` TRONG PHÁO ĐÀI!")
                self.publish_mission_log("CRITICAL", f"⚠️ [SỰ CỐ TÀI SẢN]: Đặc vụ `{model}` (Role: {role}) không tồn tại trong Thư viện. Cần Master Pull ngay thưa Ngài!")
                continue

            # 🛑 KIỂM TRA TRẠNG THÁI NẠP thưa Master
            if any(model in m for m in loaded_models):
                logger.info(f"✅ [GUARDIAN] Đặc vụ `{model}` đã có mặt tại vị trí.")
                continue

            try:
                logger.info(f"📡 [GUARDIAN] Đang triệu hồi: {model} (Role: {role})...")
                ...
                await client.post(f"{self.ollama_host}/api/generate", json={
                    "model": model, "prompt": "", "keep_alive": keep_alive, "options": cfg.get('options', {})
                }, timeout=600.0)
            except Exception as e:
                logger.error(f"❌ [GUARDIAN] Trục trặc khi triệu hồi {model}: {e}")
            
    # 💎 Alias for backward compatibility thưa Master
    warmup_models = warmup_all_models

    async def flush_gpu_memory(self, task_id="system"):
        """🧹 [SOVEREIGN ARBITRATOR]: Xả toàn bộ nơ-ron GPU để dọn đường cho Xưởng vẽ thưa Master."""
        self.publish_mission_log("VRAM_FLUSH", "🧹 [SOVEREIGN]: Đang thực hiện Surgical Flush để giải phóng VRAM...", task_id)
        client = self._get_client()
        try:
            # 🔍 Lấy danh sách các model đang cư trú trong VRAM
            resp = await client.get(f"{self.ollama_host}/api/ps")
            if resp.status_code == 200:
                models = resp.json().get("models", [])
                for m in models:
                    name = m["name"]
                    # Xả tất cả model để đảm bảo VRAM sạch tuyệt đối cho SDXL
                    self.publish_mission_log("VRAM_FLUSH", f"♻️ [SOVEREIGN]: Đang giải phóng: {name}", task_id)
                    await client.post(f"{self.ollama_host}/api/chat", json={
                        "model": name, "messages": [], "keep_alive": 0
                    })
            self.publish_mission_log("VRAM_FLUSH", "✨ [SOVEREIGN]: VRAM đã được thanh lọc thưa Master!", task_id)
            return True
        except Exception as e:
    ...
    async def restore_neural_corps(self, task_id="system"):
        """🚀 [SOVEREIGN ARBITRATOR]: Tái triệu hồi các quân đoàn nơ-ron thưa Master."""
    ...
    def get_role_config(self, role):
        ...
        # 🛡️ GIAO THỨC DỰ PHÒNG 3 TẦNG: Tuân thủ tuyệt đối rule_hardware.md thưa Master
        if not role_data:
            logger.warning(f"⚠️ [ENGINE] Role '{role}' chưa được định nghĩa. Đang kích hoạt Giao thức Chuyển hướng...")
    ...
    def load_software_rules(self):
        """📂 Trích xuất API keys và Base URLs từ rules_software.md thưa Master."""
        configs = {}
        paths = [
    ...
    async def call_chat(self, messages, role='CHAT', model=None, json_mode=False, schema=None, options=None, profile=None, keep_alive=None, task_id=None, images=None, **kwargs):
        # 🏛️ [MICROSERVICE-ROUTING]: Giao thức điều hướng trung tâm đa tầng thưa Master
        # 🧪 [SELF-AWARENESS]: Chỉ chuyển hướng nếu không phải là tự gọi chính mình thưa Master
        services = [
        ...
        for service_url, service_name in services:
            # 🛑 Nếu đây là chính dịch vụ hiện tại, ta dừng chuyển hướng và tự xử lý thưa Master
            if service_url == self.current_service_url or (self.is_brain_service and service_name == "BRAIN"):
                break 
                
            try:
                # 🛡️ [PRE-FLIGHT-CHECK]: Kiểm tra sức khỏe dịch vụ thưa Master
                client = self._get_client()
                health = await client.get(f"{service_url}/health", timeout=5.0)
                if health.status_code != 200:
                    logger.warning(f"⚠️ [PRE-FLIGHT] Service {service_name} ({service_url}) đang bận hoặc lỗi. Đang chuyển hướng...")
                    continue
                    
                # 🔒 [NEURAL-AUDIT]: Kiểm tra xem có xung đột GPU không thưa Master
                r = self._get_redis()
                if r and r.get("lock:gpu_vram"):
                    # Nếu GPU đang bị khóa bởi tác vụ quan trọng, ta sẽ chờ thay vì gây xung đột thưa Master
                ...
                logger.error(f"❌ [ENGINE-CHAT-ERR]: {e}")
                continue
                
        # 🕒 [TIME-INJECTION]: Giao thức thời gian thực thưa Master
        import datetime
        import pytz
        ...
        # 🧬 [DYNAMIC-MIND-FORGING]: Giao thức Não Vô Biên thưa Master
        if not kwargs.get('skip_forge'):
            try:
                if not self.agent_profiles_cache: self._load_agent_profiles()
                # Chọn ra 3 hồ sơ Đặc vụ phù hợp nhất để hợp nhất thưa Ngài
                ...
                if relevant_profiles:
                    ...
                    self._publish_thought(role, "🧠 [SINGULARITY]: Đã đúc kết Hệ tư tưởng Đa tầng thưa Master.", task_id)
            except: pass

        # 👁️ [AUTO-CONTEXT-INJECTION]: Giao thức Thấu thị Vĩnh cửu thưa Master
        if not kwargs.get('skip_memory'):
            try:
                # Trích xuất goal từ tin nhắn cuối cùng để tìm kiếm thưa Ngài
                ...
                if len(query) > 10:
                    ...
                    if vector:
                        ...
                        if memories:
                            ...
                            self._publish_thought(role, "👁️ [OMNIPRESENT]: Đã nạp di sản tri thức từ Qdrant thưa Master.", task_id)
            except: pass
        ...
        if estimated_tokens > 8000 and gemini_key and not any(final_model.lower().startswith(p) for p in ['gemini-', 'gpt-', 'claude-']):
            self.publish_mission_log("INFO", f"🔄 Ngữ cảnh lớn ({estimated_tokens} tokens) vượt quá giới hạn local. Tự động chuyển hướng sang Gemini 3.5 Flash thưa Master.")
        ...
        if is_cloud:
            ...
            if not prov_key:
                ...
                self.publish_mission_log("WARN", f"⚠️ Không tìm thấy API Key cho {cloud_provider.upper()} trong rules_software.md. Chuyển sang dùng model local thưa Master.")
                is_cloud = False

        
        # 💎 [PROFILE-INJECTION]: Hợp nhất cấu hình Profile nếu có thưa Master
        ...
        if target_profile and target_profile.upper() in self.profiles:
            ...
            # Chỉ ghi đè các tham số chưa có thưa Master
            for k, v in preset.items():
                if k.lower() not in final_options:
                    final_options[k.lower()] = v
        
        # 🛡️ [CPU-OFFLOAD]: Ép chạy trên CPU nếu là tác vụ giám sát nhẹ hoặc được yêu cầu thưa Master
        is_small = any(k in final_model.lower() for k in ["1.5b", "0.5b", "tiny", "phi3", "phi4-mini"])
        if kwargs.get('cpu_only') or is_small:
            final_options['num_gpu'] = 0
            # Giảm bớt ctx nếu chạy CPU để tiết kiệm RAM thưa Master
            if final_options.get('num_ctx', 4096) > 2048:
                final_options['num_ctx'] = 2048
            # Giới hạn thread để tránh làm treo host thưa Master
            if final_options.get('num_thread', 0) > 4 or final_options.get('num_thread', 0) == 0:
                final_options['num_thread'] = 4
            
        final_keep_alive = keep_alive or role_cfg.get('keep_alive', '5m')
        
        # 🧪 [TYPE-PURIFICATION]: Đảm bảo keep_alive là integer nếu là số thưa Master
        try:
            if str(final_keep_alive).replace('-', '').isdigit():
                final_keep_alive = int(final_keep_alive)
        except: pass

        # 🖼️ [NEURAL-VISION]: Đính kèm hình ảnh vào tin nhắn cuối cùng nếu có thưa Master
        ...
        # 🧪 [PARAM-PURIFICATION]: Lọc bỏ các tham số rác thưa Master
        safe_options = {k: v for k, v in final_options.items() if k not in ['profile']}
        
        # 🧪 Giao thức truyền tin Elite thưa Master
        payload = {
            'model': final_model, 
            'messages': messages, 
            'stream': True
        ...
        # vì nó sẽ làm tắt chức năng tư duy thưa Master.
        is_reasoning_model = any(k in final_model.lower() for k in ["r1", "thinking", "deepseek-v3"])
        if is_reasoning_model:
            # Reasoning models perform better with natural language prompts thưa Master
        ...
        # 🛡️ [SMALL MODEL GUARD]: Tự động áp dụng thiết lập an toàn cho các mô hình yếu thưa Master
        ...
        try:
            ...
            # 🛡️ [SYNAPSE WATCHDOG]: Theo dõi phản hồi đầu tiên từ nơ-ron thưa Master
            first_token_received = False
            waiting_start = now
            
            # 🧬 [NEURAL GATEWAY]: Phân phối luồng theo tài nguyên thưa Master
            hw_type = str(role_cfg.get('hardware', 'GPU')).upper()
            is_gpu = 'GPU' in hw_type
            
            # 🧪 [PRE-FLIGHT CHECK]: Kiểm tra sự hiện diện của nơ-ron thưa Master
            if not is_cloud:
                try:
                    ps_resp = await client.get(f'{self.ollama_host}/api/ps')
                    if ps_resp.status_code == 200:
                        ps_data = ps_resp.json()
                        is_loaded = any(final_model in m.get('name', '') for m in ps_data.get('models', []))
                        if not is_loaded:
                            self._publish_thought(role, f"🧠 [NEURAL-SUMMON]: Đặc vụ `{final_model}` hiện không có mặt. Đang triệu tập gấp thưa Master... (Có thể mất 30-60s)", task_id)
                except: pass

            # 🛡️ [PRE-FLIGHT-ABORT]: Kiểm tra lệnh dừng trước khi khởi động nơ-ron thưa Master
            r_pre = self._get_redis()
            if r_pre and r_pre.get("agent:stop_signal") in [b'true', 'true']:
                self._publish_thought(role, "🛑 [ABORT]: Lệnh Dừng đang hoạt động. Từ chối khởi động nơ-ron thưa Master.", task_id)
                return "Mission aborted by Master."

            # 🔒 Chỉ khóa GPU để bảo vệ VRAM, CPU được phép chạy song song thưa Master
            use_gpu_lock = is_gpu and not is_cloud
            if not use_gpu_lock or await self._acquire_neural_lock("gpu_vram", timeout=kwargs.get('lock_timeout')):
                try:
                    if is_cloud:
                        ...
                        req_url = f'{self.ollama_host}/api/chat'
                        req_headers = None
                        req_payload = payload
                    ...
                    async with client.stream('POST', req_url, headers=req_headers, json=req_payload, timeout=900.0) as resp:
                        if resp.status_code != 200:
                            ...
                            err_msg = f"Error: [API-ERR] Server/API trả về mã {resp.status_code} thưa Master."
                        ...
                        async for line in resp.aiter_lines():
                            if not line: 
                                # 📡 Nhịp đập nơ-ron: Báo cáo nếu đang chờ quá lâu thưa Master
                                now = time.time()
                            ...
                            # 🛡️ [SIGNAL-INTERRUPT]: Kiểm tra lệnh dừng khẩn cấp thưa Master
                            if now - last_signal_check > 2.0:
                                ...
                                if r:
                                    ...
                                    if stop_sig in [b'true', 'true']:
                                        self._publish_thought(role, "🛑 [SIGNAL]: Đã nhận lệnh Dừng khẩn cấp từ Master. Ngắt kết nối nơ-ron ngay lập tức thưa Master.", task_id)
                                        break

                            # 🛡️ [DEGENERATION CHECK]: Phát hiện vòng lặp vô tận thưa Master
                            if len(full_content) > 1000 and len(set(full_content[-100:])) < 5:
                                self._publish_thought(role, "⚠️ [NEURAL-DEGENERATION]: Phát hiện nơ-ron bị lặp (looping). Ngắt kết nối để bảo vệ VRAM.", task_id)
                                break
                            ...
                            # 🧠 [UNIFIED REASONING ENGINE]: Xử lý cả reasoning_content và <think> tag thưa Master
                            if reasoning_token:
                                if not is_thinking:
                                    is_thinking = True
                            ...
                            if token:
                                # 🧬 [CHUNK-AGNOSTIC DETECTION]: Phát hiện thẻ <think> bất kể bị chia nhỏ thưa Master
                            ...
                            # 📡 [REAL-TIME OBSERVABILITY]: Cập nhật tiến độ tư duy mỗi 1.5 giây thưa Master
                            if is_thinking:
                                now = time.time()
                                if now - last_log_time > 1.5:
                                    display_thought = thinking_content.replace("<think>", "").strip()
                                    if display_thought:
                                        msg = f"💭 [ĐANG TƯ DUY]: {display_thought[-150:]}..."
                                        if msg != last_published_text:
                                            self._publish_thought(role, msg, task_id, stream_id=stream_id)
                                            last_published_text = msg
                                            last_log_time = now
                            else:
                                # Cập nhật soạn thảo văn bản mỗi 4 giây
                                now = time.time()
                                if now - last_log_time > 4.0 and full_content.strip():
                                    display_text = full_content[-150:].strip().replace('\n', ' ')
                                    msg = f"✍️ [ĐANG SOẠN THẢO]: {display_text}..."
                                    if msg != last_published_text:
                                        self._publish_thought(role, msg, task_id, stream_id=stream_id)
                                        last_published_text = msg
                                        last_log_time = now
                        ...
                        if thinking_match:
                            ...
                            # Chỉ lọc bỏ nếu phần còn lại có nội dung đáng kể thưa Master
                            ...
                            if len(stripped_content) > 20:
                                ...
                                # Nếu model chỉ "nghĩ" mà không "nói" ngoài thẻ, lấy phần nghĩ làm kết quả thưa Master
            ...
            # 🛡️ [JSON THOUGHT EXTRACTION] - Trích xuất từ trường JSON (thought/reasoning)
            if json_mode:
                try:
            ...
            if json_mode or schema:
                try: 
                    # 💎 [ROBUST JSON EXTRACTION]: Chỉ trích xuất khi được yêu cầu thưa Master
                    return self._extract_json_from_text(full_content)
                except: 
                    self._publish_thought(role, f"⚠️ Lỗi parse JSON. Trả về text thô.", task_id)
                    return full_content
            
            # 📜 [LEGACY-COMPATIBILITY]: Luôn trả về chuỗi văn bản cho các Đặc vụ cũ thưa Master
    ...
    def _extract_json_from_text(self, text):
        """Trích xuất JSON từ văn bản thô một cách bền bỉ thưa Master."""
        if not text: return {}
    ...
    def call_skill(self, skill_id, params, task_id="system"):
        ...
        if not skill_folder:
            ...
            self._publish_thought("WARN", f"⚠️ Không tìm thấy mapping cho `{skill_id}`, kích hoạt Trinh sát nơ-ron...", task_id)
