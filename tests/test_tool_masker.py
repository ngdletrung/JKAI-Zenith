...
class TestToolMasker(unittest.TestCase):
    ...
    def test_greeting_masks_to_zero(self):
        tools, _ = mask_tools(goal="Xin chào", intent="SOCIAL", skill="GREETING", all_tools=self.mock_tools)
        self.assertEqual(len(tools), 0)

    def test_web_search_masking(self):
        tools, _ = mask_tools(goal="tìm kiếm tin tức AI", skill="SEARCH_WEB_GLOBAL", all_tools=self.mock_tools)
        tool_names = [t["name"] for t in tools]
        self.assertIn("search_web", tool_names)
        self.assertNotIn("run_command", tool_names)

    def test_file_view_masking(self):
        tools, _ = mask_tools(goal="xem file core.py", all_tools=self.mock_tools)
        tool_names = [t["name"] for t in tools]
        self.assertIn("view_file", tool_names)

    def test_shell_command_masking(self):
        tools, _ = mask_tools(goal="chạy lệnh docker ps", all_tools=self.mock_tools)
        tool_names = [t["name"] for t in tools]
